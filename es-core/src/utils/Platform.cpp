...
namespace Utils
{
	namespace Platform
	{
		...
		BatteryInformation queryBatteryInformation()
		{
			...
			ret.level = 0;
			bool percentSetFromCache = false;

#ifdef KNULLI
{
    // Check the file generated by batteryplus directly
    ...
    if (Utils::FileSystem::exists(pctPath)) {
        ...
        const bool digitsOnly = !s.empty() &&
            std::all_of(s.begin(), s.end(), [](unsigned char c){ return std::isdigit(c); });

        ret.hasBattery = true;
        ret.level = digitsOnly ? std::stoi(s) : 0; // non-numeric -> 0%
        percentSetFromCache = true;
    }
    // If the file is missing we'll let the sysfs fallback below compute percent for this poll only
}
#endif
